...
test("TikTok implementation remains isolated from recognition models and browser code contains no server token secrets", () => {
  const files = ["src/lib/tiktok/client.ts", "src/lib/tiktok/caption.ts"].map((file) => readFileSync(file, "utf8")).join("\n");
...
test("future Direct Post implementation retains its server-side safeguards", () => {
...
test("future Direct Post implementation retains creator and privacy restrictions", () => {
...
test("expired authorization clears the protected connection", () => {
...
test("customer-facing TikTok posting is a static Coming soon state with no OAuth or API path", () => {
  assert.match(postingUi, /Post to TikTok/);
  assert.match(postingUi, /Coming soon/);
  assert.match(postingUi, /aria-disabled="true"/);
  assert.doesNotMatch(postingUi, /<button|connectTikTokOAuth|getTikTokConnection|getTikTokCreatorInfo|initializeTikTokPost|uploadTikTokVideo|fetch\(/u);
  assert.doesNotMatch(postingUi, /not configured|TIKTOK_CLIENT_SECRET|TIKTOK_REDIRECT_URI|docs\/TIKTOK_SETUP\.md/u);
  assert.match(readFileSync("src/components/editor-workspace.tsx", "utf8"), /Download video/);
});
