...
import { mediaFileError } from "../src/lib/editor/media.ts";
...
test("the initial empty state is Quran-specific, keyboard-accessible, and accepts drag/drop", () => {
  for (const copy of ["Upload your recitation", "Add a Quran recitation and we&apos;ll detect the verses, sync the captions, and prepare them for editing.", "Drop your recitation here", "or click to choose a file", "MP4, MOV, MP3, WAV, M4A and more"]) assert.ok(workspace.includes(copy));
...
test("browse and drag/drop share type validation without an arbitrary source-size cap", () => {
  assert.equal(mediaFileError({ type: "audio/mpeg", size: 2 * 1024 * 1024 * 1024 }), null);
