...
type PlanComparisonDialogProps = { entitlements: AccountEntitlements; session?: Session | null; onClose: () => void; onEntitlementsRefresh?: () => Promise<void>; checkoutReturn?: boolean };
...
export default function PlanComparisonDialog({ entitlements, session = null, onClose, onEntitlementsRefresh, checkoutReturn = false }: PlanComparisonDialogProps) {
  const dialogRef = useRef<HTMLDivElement>(null);
  const [interval, setInterval] = useState<BillingInterval>("month");
  const [billing, setBilling] = useState<BillingStatus | null>(null);
  const [message, setMessage] = useState<string | null>(checkoutReturn ? "Updating your plan…" : null);
  ...
  useEffect(() => {
    if (!checkoutReturn || !session) return;
    ...
    const refreshAfterCheckout = async () => {
      try {
        ...
        if (!active) return;
        setBilling(next);
        if (next.plan !== "free") { setMessage("Your plan is active."); return; }
      } catch { /* A retry button remains available if the webhook is delayed. */ }
      attempts += 1;
      if (active && attempts < 6) window.setTimeout(() => { void refreshAfterCheckout(); }, 2_000);
      else if (active) setMessage("Your payment return was received. Stripe is still updating your plan; use Refresh in a moment.");
    };
    void refreshAfterCheckout();
    return () => { active = false; };
  }, [checkoutReturn, onEntitlementsRefresh, session]);
  ...
  return <div className="plan-comparison-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="plan-comparison-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="plan-comparison-title" onKeyDown={(event) => event.stopPropagation()}>
    ...
    <footer><span>Payments and plan changes are securely managed by Stripe.</span><div>{checkoutReturn && session && <button className="editor-button editor-button-quiet" type="button" onClick={() => void refresh()}>Refresh</button>}<button className="editor-button editor-button-quiet" type="button" onClick={onClose}>Done</button></div></footer>
  </section></div>;
}
