...
export default function AccountSettings() {
  ...
  if (!session) return <DashboardShell current="account"><section className="account-page"><div className="account-card"><p className="account-eyebrow">ACCOUNT SETTINGS</p><h1>Sign in to manage your account</h1><p>Account settings are available to signed-in Quran AutoCaption members.</p><Link className="account-action" href="/create">Create a video</Link></div></section></DashboardShell>;
  ...
  return <DashboardShell current="account"><section className="account-page"><div className="account-card">
    ...
    <section><h2>Videos</h2><dl><div><dt>Saved videos</dt><dd>{usage ? `${usage.project_count} saved${entitlements.cloudProjectLimit === null ? "" : ` of ${entitlements.cloudProjectLimit}`}` : "Loading saved videos…"}</dd></div>{entitlements.cloudProjectLimit !== null && <div><dt>Free plan</dt><dd>Keep your {entitlements.cloudProjectLimit} most recent saved videos</dd></div>}</dl><Link className="account-secondary account-section-link" href="/videos">View videos</Link></section>
