...
export default function EditorWorkspace(props: EditorWorkspaceProps) {
  ...
  useEffect(() => {
    if (!session || new URLSearchParams(window.location.search).get("billing") !== "success") return;
    const frame = window.requestAnimationFrame(() => {
      setCheckoutReturn(true); setPlanComparisonOpen(true);
  ...
  return <div className={`editor-shell ${layoutResizing ? "is-resizing-layout" : ""}`} style={{ "--timeline-height": `${timelineCollapsed ? 38 : timelineHeight}px`, "--left-panel-width": `${leftPanelWidth}px`, "--right-panel-width": `${rightPanelWidth}px` } as CSSProperties}>
    ...
    {planComparisonOpen && <PlanComparisonDialog entitlements={accountEntitlements} session={session} checkoutReturn={checkoutReturn} onEntitlementsRefresh={onRefreshEntitlements} onClose={() => { setPlanComparisonOpen(false); setCheckoutReturn(false); }} />}
  </div>;
}
