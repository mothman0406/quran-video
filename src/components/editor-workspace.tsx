      ...
      <section className="editor-main-stage">
        ...
        <div className="editor-canvas-well">
          {videoUrl ? <div ref={previewRef} className="project-preview-canvas editor-canvas" data-project-aspect-ratio={selectedFormatDefinition.aspectRatio} data-project-format={projectFormat.preset} style={{ aspectRatio: `${projectFormat.width} / ${projectFormat.height}` }} onPointerDown={onCanvasBackgroundPointerDown}>
            ...
            <CaptionPreview videoRef={videoRef} segments={segments} content={content} typography={typography} captionBackground={captionBackground} positioning={positioning} transitionSettings={transitionSettings} showVerseNumber={showVerseNumber} selectedObject={selectedObject} onSelectObject={onSelectObject} onObjectPointerDown={onObjectPointerDown} onResizePointerDown={onResizePointerDown} onPointerMove={onObjectPointerMove} onPointerUp={onObjectPointerUp} />
        ...
        {(busy || (stage === "complete" && alignments.length > 0) || showCorrection || errorMessage || timingWarning || exportState) && <div className="editor-notices">
          ...
          {process.env.NODE_ENV !== "production" && ctcShadowCompleted && <div className="editor-notice editor-notice-success"><strong>CTC forced alignment completed</strong><span>It supplies the globally ordered scaffold when Whisper has no word timestamps.</span></div>}
        ...
        <div className="editor-sidebar-scroll">
          {selectedObject ? <>
            ...
            {selectedObject === "arabic" ? <>
              ...
              <div className="editor-color-row"><label>Text color</label><input type="color" value={typography.textColor} onChange={(event) => updateObjectTypography("textColor", event.target.value)} /></div>
