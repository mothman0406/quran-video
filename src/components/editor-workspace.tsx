...
import { exportQualityPreset, type ExportQuality } from "@/lib/export/quality";
...
export default function EditorWorkspace(props: EditorWorkspaceProps) {
  ...
  const {
    ...
    onOpenCloudProjects, onSessionChange, onPlanChange, onDiscard, onNewProject, onExportOpen, onExport, onRecheckExport, onExportPreflightAction, onCancelExport, onDownloadExport,
  ...
  return <div className="editor-shell" style={{ "--timeline-height": `${timelineCollapsed ? 38 : timelineHeight}px` } as CSSProperties}>
    ...
    <div className={`editor-body ${leftCollapsed ? "is-left-collapsed" : ""} ${rightCollapsed ? "is-right-collapsed" : ""} ${timelineCollapsed ? "is-timeline-collapsed" : ""}`}>
      ...
      <section className="editor-main-stage">
        ...
        {(busy || (stage === "complete" && alignments.length > 0) || showCorrection || errorMessage || timingWarning || exportState) && <div className="editor-notices">
          ...
          {exportState === "complete" && exportResult ? <div className="editor-export-complete" role="status"><div><strong>✓ Export complete</strong><span>{exportResult.width} × {exportResult.height}</span><span>{exportResult.mimeType.split(";")[0]?.replace("video/", "").toUpperCase()} · {playbackRateLabel(exportResult.playbackRate)} · {formatFileSize(exportResult.fileSizeBytes)}</span>{exportIsStale && <small>Project changed since this export.</small>}</div><div className="editor-export-complete-actions"><button className="editor-button editor-button-accent" type="button" onClick={onDownloadExport}>Download video</button><button className="editor-button editor-button-quiet" type="button" onClick={onExportOpen}>Export another version</button></div></div> : exportState && exportState !== "complete" && <div className="editor-notice"><strong>{exportState === "error" ? "Export stopped" : `Exporting · ${exportState.phase}`}</strong><span>{exportError ?? "Source media is processed locally."}</span></div>}
        </div>}
      </section>
    ...
    {exportOpen && <div className="editor-modal-backdrop" role="dialog" aria-modal="true" aria-label="Export video"><div className="editor-modal"><div className="editor-modal-heading"><div><SectionLabel>{exportState === "error" ? "Export error" : "Export preflight"}</SectionLabel><h2>{exportState === "error" ? "Export couldn't start" : exportPreflight?.status === "blocked" ? "Export needs attention" : "Things to review"}</h2></div><button type="button" onClick={() => onSetExportOpen(false)}>×</button></div>{exportState !== "error" && <><div className="editor-export-grid"><label>Quality<select className="editor-select" value={exportQuality} disabled={Boolean(exportState && typeof exportState === "object")} onChange={(event) => onSetExportQuality(event.target.value as ExportQuality)}><option value="basic">Basic · 720p</option><option value="standard">Standard · 1080p</option><option value="ultra">Ultra · 4K</option></select></label><div><SectionLabel>Resolution</SectionLabel><strong>{exportFormat.width} × {exportFormat.height}</strong><small>{playbackRateLabel(playbackRate)} · {exportQualityPreset(exportQuality).watermarkRequired ? "Small watermark included" : exportQualityPreset(exportQuality).description}</small></div><div><SectionLabel>Output</SectionLabel><strong>{outputPlan?.profile?.container.toUpperCase() ?? "MP4 preferred"}</strong><small>Processed locally</small></div></div>{!exportPreflight ? <p className="editor-muted" role="status">Checking export requirements…</p> : exportPreflight.checks.filter((item) => item.severity !== "pass").map((item) => <div className={item.severity === "blocking" ? "editor-notice editor-notice-error" : "editor-platform-warning"} key={`${item.id}:${item.affectedSegmentId ?? "project"}`}><strong>{item.severity === "blocking" ? "✕" : "⚠"} {item.title}</strong><span>{item.message}</span>{item.action && <button className="editor-button editor-button-quiet" type="button" onClick={() => onExportPreflightAction(item.action!, item.affectedSegmentId)}>{item.action === "move-to-safe-area" ? "Move to safe area" : item.action === "relink-source" ? "Relink source" : "Review"}</button>}</div>)}</>}<div className="editor-modal-actions">{exportState === "error" ? <button className="editor-button editor-button-accent" type="button" onClick={onExportOpen}>Try again</button> : <>{exportPreflight?.status !== "ready" && <button className="editor-button editor-button-quiet" type="button" disabled={Boolean(exportState && typeof exportState === "object")} onClick={onRecheckExport}>Recheck</button>}{exportPreflight?.status === "warnings" && <button className="editor-button editor-button-accent" type="button" disabled={Boolean(exportState && typeof exportState === "object")} onClick={onExport}>Export anyway</button>}</>}{exportState && typeof exportState === "object" && <button className="editor-button editor-button-quiet" type="button" onClick={onCancelExport}>Cancel</button>}</div>{exportError && <p className="editor-alert">{exportError}</p>}{exportDiagnostics && <details className="editor-diagnostics"><summary>Export diagnostics</summary><p>{exportDiagnostics.outputContainer} · {exportDiagnostics.renderedFrameCount} frames · {exportDiagnostics.effectiveRenderingFps.toFixed(1)} fps · {playbackRateLabel(playbackRate)}</p></details>}</div></div>}
  </div>;
}
