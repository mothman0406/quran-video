    ...
    <main className="min-h-screen bg-[#f5f2eb] px-5 py-10 text-[#17211b] sm:px-8">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {result && <section className="rounded-2xl border border-[#d8d5cc] bg-[#fbfaf6] p-5">
          ...
          {analysis?.diagnostics.some((diagnostic) => diagnostic.rejectionReason) && <details className="mt-4 text-sm text-[#68716a]"><summary className="cursor-pointer font-semibold text-[#35604f]">Matcher diagnostics</summary>{analysis.diagnostics.map((diagnostic) => <p className="mt-2" key={`${diagnostic.startMs}-${diagnostic.endMs}`}>Top candidate: {diagnostic.topCandidate ? `${diagnostic.topCandidate.startVerseKey} – ${diagnostic.topCandidate.endVerseKey} (${Math.round(diagnostic.topCandidate.confidence * 100)}%; score ${diagnostic.topCandidate.score}, text ${diagnostic.topCandidate.textSimilarity}, sequence ${diagnostic.topCandidate.tokenSequenceSimilarity})` : "none"}. Candidate path: {diagnostic.candidateGenerationPath ?? "not run"}. Rejection: {diagnostic.rejectionReason ?? "none"}.</p>)}</details>}
