...
export default function RecognitionSpikePage() {
  ...
  function timingDebugPayload() {
    ...
    return {
      ...
      transitions: analysis?.timingTrace?.transitions ?? [],
  ...
  function timingReportText() {
    ...
    return [
      ...
      "", "AYAH TRANSITION CANDIDATES", ...debug.transitions.flatMap((transition) => [
        `${transition.previousVerseKey} → ${transition.nextVerseKey}: corridor ${value(transition.searchCorridor.startMs)}–${value(transition.searchCorridor.endMs)}; selected ${value(transition.selectedTransitionMs)}`,
        ...transition.candidateNextAyahEvidence.map((candidate) => `${value(candidate.timestampMs)} | word ${candidate.canonicalWordIndex} | confidence ${candidate.confidence} | ${candidate.evidenceType} | VAD onset nearby: ${candidate.vadSpeechOnsetNearby ? "yes" : "no"} | ${candidate.accepted ? "accepted" : "rejected"}: ${candidate.reason}`),
      ]),
