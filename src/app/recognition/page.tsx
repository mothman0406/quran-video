    ...
    <main className="min-h-screen bg-[#f5f2eb] px-5 py-10 text-[#17211b] sm:px-8">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {result && <section className="rounded-2xl border border-[#d8d5cc] bg-[#fbfaf6] p-5">
          ...
          {analysis && <details className="mt-4 text-sm text-[#68716a]"><summary className="cursor-pointer font-semibold text-[#35604f]">Whole-passage diagnostics</summary><p className="mt-2">Decision: {analysis.passage.state}; margin: {analysis.passage.candidateMargin ?? "n/a"}; later chunks disambiguated: {analysis.passage.disambiguatedByLaterChunks ? "yes" : "no"}.</p>{analysis.passage.candidates.map((candidate) => <p className="mt-2" key={`${candidate.startVerseKey}-${candidate.endVerseKey}`}>{candidate.startVerseKey} – {candidate.endVerseKey}: score {candidate.totalScore}, confidence {Math.round(candidate.confidence * 100)}%, text {candidate.textSimilarity}, sequence {candidate.sequenceConsistency}, coverage {candidate.transcriptCoverage}, consecutive ayat {candidate.consecutiveAyat}.</p>)}{analysis.diagnostics.map((diagnostic) => diagnostic.rejectionReason ? <p className="mt-2" key={`${diagnostic.startMs}-${diagnostic.endMs}`}>Chunk {formatTime(diagnostic.startMs)}–{formatTime(diagnostic.endMs)}: {diagnostic.rejectionReason}; local candidate {diagnostic.topCandidate ? `${diagnostic.topCandidate.startVerseKey} – ${diagnostic.topCandidate.endVerseKey}` : "none"}.</p> : null)}</details>}
