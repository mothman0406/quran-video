...
export function refineFirstAyahOnsetWithEnergy(
  ...
  if (!analysis.rms.length) return null;
  const fromMs = Math.max(0, minimumStartMs, anchorStartMs - Math.max(0, lookbackMs));
  ...
  return closestQuietEnd === null ? null : Math.max(Math.round(minimumStartMs), Math.round(closestQuietEnd));
}
