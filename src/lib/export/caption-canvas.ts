import { arabicCaptionPresentationWords, composeArabicCaptionText, captionVisualStatesAtTime, linkedCaptionStackLayout, resolveWordHighlightPresentation, translationDisplayText, type ArabicPresentationWord } from "../editor/captions.ts";
...
function drawArabicWords(context: CanvasRenderingContext2D, lines: readonly ArabicWordLine[], x: number, y: number, lineHeight: number, fill: string, highlightColor: string, highlightIntensity: number, highlightOpacity: number, scale: number, outline: boolean, outlineWidth: number, outlineColor: string, shadow: boolean, shadowBlur: number, shadowStrength: number) {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    for (const [index, word] of line.words.entries()) {
      ...
      const highlight = resolveWordHighlightPresentation({ baseTextColor: fill, highlightColor, intensity: highlightIntensity, isHighlighted: word.highlighted });
      context.fillStyle = word.highlighted ? alphaColor(highlight.color, highlightOpacity) : highlight.color;
      context.shadowColor = word.highlighted ? highlight.glowColor : shadow ? `rgba(0,0,0,${shadowStrength})` : "transparent";
      context.shadowBlur = word.highlighted ? highlight.glowBlurPx * scale : shadow ? shadowBlur : 0;
      context.shadowOffsetY = word.highlighted ? 0 : shadow ? 2 : 0;
...
export function drawExportCaptions(context: CanvasRenderingContext2D, request: LocalExportRequest, timeMs: number, arabicFont: string) {
  ...
  for (const state of captionVisualStatesAtTime(request.segments, timeMs, request.transitionSettings)) {
    ...
    context.font = `${translationTypography.translationItalic ? "italic " : ""}${translationTypography.translationFontWeight} ${translationSize}px ${translationTypography.translationFontFamily}`; const translationLines = translation ? wrap(context, translation, resolvedPositioning.translationPositionLinked ? maxWidth : translationMaxWidth) : []; const translationHeight = translationLines.length * translationSize * 1.25;
    ...
    let cursor = y;
    context.font = `${arabicSize}px "${arabicFont}", serif`; if (arabicWordLines) drawArabicWords(context, arabicWordLines, x, cursor + arabicSize, arabicSize * resolvedTypography.arabicLineSpacing, alphaColor(resolvedTypography.textColor, resolvedTypography.arabicOpacity), resolvedTypography.wordHighlightColor, resolvedTypography.wordHighlightIntensity, resolvedTypography.arabicOpacity, scale, resolvedTypography.arabicOutlineEnabled, resolvedTypography.arabicOutlineWidth * scale, resolvedTypography.arabicOutlineColor, resolvedTypography.arabicShadowEnabled, resolvedTypography.arabicShadowBlur * scale, resolvedTypography.arabicShadowStrength); else drawText(context, arabicLines ?? [], x, cursor + arabicSize, arabicSize * resolvedTypography.arabicLineSpacing, resolvedTypography.textAlign, "rtl", alphaColor(resolvedTypography.textColor, resolvedTypography.arabicOpacity), resolvedTypography.arabicOutlineEnabled, resolvedTypography.arabicOutlineWidth * scale, resolvedTypography.arabicOutlineColor, resolvedTypography.arabicShadowEnabled, resolvedTypography.arabicShadowBlur * scale, resolvedTypography.arabicShadowStrength); cursor += arabicHeight;
    if (translationLines.length && resolvedPositioning.translationPositionLinked) { cursor += resolvedTypography.translationSpacingBelowArabic * scale; context.font = `${translationTypography.translationItalic ? "italic " : ""}${translationTypography.translationFontWeight} ${translationSize}px ${translationTypography.translationFontFamily}`; drawText(context, translationLines, x, cursor + translationSize, translationSize * 1.25, translationTypography.translationTextAlign, "ltr", alphaColor(translationTypography.translationTextColor, translationTypography.translationOpacity), translationTypography.translationOutlineEnabled, translationTypography.translationOutlineWidth * scale, translationTypography.translationOutlineColor, translationTypography.translationShadowEnabled, translationTypography.translationShadowBlur * scale, translationTypography.translationShadowStrength); cursor += translationHeight; }
