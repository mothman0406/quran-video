...
- Audited the exact `@ffmpeg/ffmpeg` 0.12.15 `FFmpeg.load()` contract. `classWorkerURL` overrides its essential wrapper worker; `workerURL` is only for a multi-thread core. The prior configuration self-hosted the wrapper's ESM worker and helper modules even though this Webpack build can bundle them. That unnecessary override was the production initialization incompatibility boundary.
- Restored the supported architecture: Next/Webpack now owns the wrapper worker and all of its ESM helper imports, while the single-thread `@ffmpeg/core` 0.12.10 UMD JS/WASM remain the only two same-origin public assets. The production build proved that its bundled worker is classic; passing it an ESM core produces `Error: Cannot find module '/ffmpeg/ffmpeg-core.js'` after the worker's unsupported dynamic-import fallback. `FFmpeg.load()` now receives exactly `{ coreURL, wasmURL }` from the mutually compatible UMD pair and supplies neither worker URL. The upstream v12.15 release contains the 0.12.10 core, so dependency versions remain deliberately pinned.
- Kept FFmpeg lazy and added a production-safe initialization diagnostic. With `?debugMedia=1`, a failure reports its sanitized exception name/message, source substage, main-thread versus wrapper-worker origin, worker-message-protocol status, and an FFmpeg-only source location when present. The wrapper worker's ERROR payload is now preserved for the trace rather than collapsed before logging.
- Updated core-only package-byte checks and focused regression coverage for the supported load configuration, UMD source paths, safe redaction/classification, native laziness, fallback initialization-before-normalization, and the existing no-upload/resource-policy constraints.

Verification: `npm run regression:quran` passes production invariant probes and refreshed `docs/regression/results/20260912.md`; its optional local real quran-align/EveryAyah benchmark remains unavailable in this workspace. `npm test` (389 passing), `npx tsc --noEmit`, `npm run lint -- --quiet`, `npm run build`, and `git diff --check` pass. A locally served production build in headless Chrome exercised the emitted Next/Webpack wrapper worker and exact `FFmpeg.load({ coreURL, wasmURL })` call successfully (`{"loaded":true}`). The build retains the existing non-fatal VAD/ONNX Runtime dynamic-import warnings and optional TikTok configuration reminder.
...
- Fixed Billing’s hydration flash: the server passes only the derived Stripe-configured boolean to `/billing`, while both Billing surfaces keep unresolved status visibly loading and only show the unconfigured copy after a resolved false state. No secret or Price ID reaches client props.
