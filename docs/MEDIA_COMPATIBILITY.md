...
The app ships only the two core runtime files with the deployment: `/ffmpeg/ffmpeg-core.js` and `/ffmpeg/ffmpeg-core.wasm`, copied byte-for-byte from `node_modules/@ffmpeg/core/dist/umd/` at `@ffmpeg/core` 0.12.10. They are root-relative same-origin paths, so on the canonical site they resolve under `/ffmpeg/` even from `/editor` or `/editor?debugMedia=1`; no retired Netlify hostname, CDN, local filesystem path, or blob URL is used. The `@ffmpeg/ffmpeg` 0.12.15 wrapper and its `dist/esm/worker.js`, `const.js`, `errors.js`, and `utils.js` dependencies remain Next/Webpack-managed exactly as its `FFmpeg.load()` implementation supports. Webpack emits that wrapper as a classic worker, so its `importScripts()` path requires the matching UMD core rather than the ESM core. The 0.12.15 upstream release tag ships the 0.12.10 single-thread core; both are deliberately pinned. The single-thread core has no `ffmpeg-core.worker.js`; `workerURL` is therefore not supplied.
...
For a production-safe compatibility trace, open the editor with `?debugMedia=1`, open the browser console, select the recording, and copy the concise `[Quran AutoCaption media]` entries. Runtime fallback emits `runtime-wrapper-import-start/ok`, `core-load-start/ok`, `wasm-load-start/ok`, `ffmpeg-initialize-config`, and `ffmpeg-initialize-start/ok`. On initialization failure, it additionally reports the exact sanitized `errorName` and `errorMessage`, the classified `initializationSubstage` (`main-thread`, `wrapper-worker`, `ffmpeg-core-factory`, `wasm-instantiation`, or `worker-message-protocol`), whether it arrived through the wrapper worker's ERROR protocol, and a safe FFmpeg-only source location when available. Worker ERROR payloads are no longer discarded. The trace otherwise includes only the visible extension, file size, playback/native-audio capability, selected route, WORKERFS result, demux/audio-decode result, stream count, codec/sample rate/channels, PCM duration, and final internal error code. It never includes the filename, media contents, audio samples, object or external URLs, account data, tokens, local filesystem paths, or raw FFmpeg logs. Mediabunny remains the first local MOV/MP4 container/track inspector and browser/WebCodecs capability check; the proven FFmpeg path needs no separate WebCodecs demux/decode implementation.
