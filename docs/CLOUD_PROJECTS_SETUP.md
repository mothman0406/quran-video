...
New local video and local audio imports each start one automatic recognition run after their source is accepted. A stable identity combining the media asset and fingerprint prevents rerender duplicates. A genuinely different source gets a new run. Restored projects that already have caption segments skip automatic recognition; restored undetected projects may start one run once their source is available. **Detect again** and **Try again** remain recovery actions, and **Correct detection** remains the manual passage authority. Production intentionally does not offer YouTube import.
